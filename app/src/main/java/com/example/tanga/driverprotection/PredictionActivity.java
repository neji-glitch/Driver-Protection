...
public class PredictionActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String speed = prefs.getString("speed", String.valueOf(random));
        speedometer.speedTo(Integer.parseInt(speed));
                    ...
                    try {
                        ...
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                final int max = 99;
                                final int random = new Random().nextInt((max - min) + 1) + min;
                                speedometer.speedTo(Float.valueOf(prefs.getString("speed", String.valueOf(random))));
...
*/
    ...
    private void situationcheck(Speedometer speedometer)
    {
        ...
        if(speedometer.getSpeed()<80 && drunk!=1)
        {
            situation = 0;
            constraintpred.setBackgroundResource(R.drawable.predictionbg);
        }
    }
        ...
        StringRequest stringRequest = new StringRequest(Request.Method.GET,UrlConst.Prediction+"km="+pt_km+"&situation="+situation+"&city="+cityval+"&weather="+meteo.getText().toString().substring(0,5) +"&fin="+end+"&debut="+begin, new Response.Listener<String>() {
            ...
            public void onResponse(String response) {
                try {
                    ...
                    if(prediction*100>=100)
                    {
prediction = 96.54;                  }
else if(prediction*100<60)
                    {
                        pred.setTextColor(Color.parseColor("#ff33b5e5"));
